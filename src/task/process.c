...
int process_load_for_slot(const char* filename, struct process** process, int process_slot) {
    ...
    // The strncpy function will be created later in the book! 
    ...
    if (ERROR_I(task) == 0) {
        res = ERROR_I(task);
        // Please note the "goto out" is missing in the commit ensure to include it here. 
        goto out;
    } 
    
    _process->task = task;

    // If youre using the published commit log, // ensure the line below appears as it is here, // not as it appears in the commit. 
...
int process_get_free_slot() { 
    // Loop through each slot in the processes array 
    for (int i = 0; i < PEACHOS_MAX_PROCESSES; i++) { 
        // If we find an empty (null) slot, return its index 
        if (processes[i] == 0) 
            return i; 
    } 
    // If we reach this point, it means all slots are taken. 
    // Return an error code to signify that no slot is available. 
    return -EISTKN; 
}
